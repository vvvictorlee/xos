...
void elastic_per_day_limit_parameters::validate() const
{
  // At the very least ensure parameters are not set to values that will cause divide by zero errors later on.
  // Stricter checks for sensible values can be added later.
  EOS_ASSERT(low_threshold.threshold > 0, resource_limit_exception, "elastic limit parameter 'threshold' cannot be zero");
  EOS_ASSERT(medium_threshold.threshold > low_threshold.threshold, resource_limit_exception, "elastic limit parameter 'medium_range' is not a well-defined range");
  EOS_ASSERT(high_threshold.threshold > medium_threshold.threshold, resource_limit_exception, "elastic limit parameter 'high_range' is not a well-defined range");
}
...
void resource_limits_manager::add_indices()
{
  ...
  _db.add_index<resource_usage_per_day_index>();
  _db.add_index<resource_limits_per_day_config_index>();
...
void resource_limits_manager::set_block_parameters(const elastic_limit_parameters &cpu_limit_parameters, const elastic_limit_parameters &net_limit_parameters, uint64_t per_day_resource_usage_unlimit_threshold)
{
  ...
  _db.modify(per_day_config, [&](resource_limits_per_day_config_object &c) {
    c.per_day_resource_usage_unlimit_threshold = per_day_resource_usage_unlimit_threshold;
  });
}
...
void resource_limits_manager::update_account_usage(const flat_set<account_name> &accounts, uint32_t time_slot)
{
  ...
  for (const auto &a : accounts)
  {
    ...
    const auto &usage_per_day = _db.get<resource_usage_per_day_object, by_owner>(a);
...
void resource_limits_manager::add_transaction_usage(const flat_set<account_name> &accounts, uint64_t cpu_usage, uint64_t net_usage, uint32_t time_slot)
{
  ...
  for (const auto &a : accounts)
  {
    ...
    _db.modify(usage_per_day, [&](auto &bu) {
      bu.net_usage.add(net_usage);
      bu.cpu_usage.add(cpu_usage);
    ...
    if (cpu_weight >= 0 && state.total_cpu_weight > 0)
    {
      ...
      auto virtual_cpu_capacity_in_window = (uint128_t)state.virtual_cpu_limit * window_size;
      ...
      EOS_ASSERT(cpu_used_in_window <= max_user_use_in_window,
                 ...
                 ("n", name(a))("cpu_used_in_window", cpu_used_in_window)("max_user_use_in_window", max_user_use_in_window));
    }
    ...
    if (net_weight >= 0 && state.total_net_weight > 0)
    {
      ...
      EOS_ASSERT(net_used_in_window <= max_user_use_in_window,
                 ...
                 ("n", name(a))("net_used_in_window", net_used_in_window)("max_user_use_in_window", max_user_use_in_window));
    }
  }
...
void resource_limits_manager::add_pending_ram_usage(const account_name account, int64_t ram_delta)
{
  ...
  _db.modify(usage_per_day, [&](auto &u) {
    u.ram_usage.add(ram_delta);
  });
}
...
std::tuple<int64_t, int64_t, int64_t> resource_limits_manager::check_account_limits(const account_name &account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) const
{
  ...
  return std::make_tuple(ram_bytes,
                         ...
                         usage_per_day.tx_usage.validate(config.per_day_resource_usage_unlimit_threshold, net_weight),

                         usage_per_day.tx_usage.validate(config.per_day_resource_usage_unlimit_threshold, cpu_weight));
}
...
void resource_limits_manager::get_account_limits(const account_name &account, int64_t &ram_bytes, int64_t &net_weight, int64_t &cpu_weight) const
{
  ...
  std::tie(ram_bytes, net_weight, cpu_weight) = check_account_limits(account,ram_bytes, net_weight, cpu_weight);
}
