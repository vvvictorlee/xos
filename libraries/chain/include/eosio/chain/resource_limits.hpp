...
namespace eosio { namespace chain { namespace resource_limits {
   namespace impl {
      ...
      template<typename T>
      struct tariff {
         static_assert(std::is_integral<T>::value, "tariffs must have integral types");
         T threshold;
         T fee;
      };
   }
   ...
   using tariff = impl::tariff<uint64_t>;
  ...
  struct elastic_per_day_limit_parameters {
      tariff    low_threshold;       ///<  lower than low threshold  fee
      tariff    medium_threshold;    ///< lower than threshold    
      tariff    high_threshold;      ///< lower than threshold  
   ...
   class resource_limits_manager {
      public:
         ...
         void set_block_parameters( const elastic_limit_parameters& cpu_limit_parameters, const elastic_limit_parameters& net_limit_parameters, uint64_t per_day_resource_usage_unlimit_threshold );
...
std::tuple<int64_t,int64_t,int64_t> check_account_limits( const account_name& account,int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) const;
