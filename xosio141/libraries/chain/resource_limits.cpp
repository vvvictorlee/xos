...
void resource_limits_manager::set_block_parameters(const elastic_limit_parameters &cpu_limit_parameters, const elastic_limit_parameters &net_limit_parameters, uint64_t resource_usage_per_day_unlimit_threshold)
{
  ...
  _db.modify(per_day_config, [&](resource_limits_per_day_config_object &c) {
    c.resource_usage_per_day_unlimit_threshold = resource_usage_per_day_unlimit_threshold;
  });
}
...
std::tuple<int64_t, int64_t, int64_t> resource_limits_manager::check_account_limits(const account_name &account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight) const
{
  ...
  return std::make_tuple(ram_bytes,
                         ...
                         usage_per_day.tx_usage.validate(config.resource_usage_per_day_unlimit_threshold, net_weight),

                         usage_per_day.tx_usage.validate(config.resource_usage_per_day_unlimit_threshold, cpu_weight));
}
